...
class PayPalDonations_Admin
{
    ...
    public function init()
    {
        ...
        add_settings_field(
            ...
            array(
                'label_for' => 'amount',
                'description' => __(
                    'The default amount for a donation (Optional).<br /> Multiple values can be set in a comma-separated list. This will work only when the Show Amount(s) option is checked.',
                    PayPalDonations::TEXT_DOMAIN
                ),
            )
        );
        add_settings_field(
            'show_amount',
            __('Show Amount(s)', PayPalDonations::TEXT_DOMAIN),
            array($this, 'showAmountCallback'),
            self::PAGE_SLUG,
            'default_section',
            array(
                'label_for' => 'show_amount',
                'description' => __(
                    'Show the amount to the users? (Optional).',
                    PayPalDonations::TEXT_DOMAIN
                ),
            )
        );
        ...
        add_settings_field(
            'button_url',
            __('Custom Button Image', PayPalDonations::TEXT_DOMAIN),
        ...
        add_settings_field(
            'button_text',
            __('Custom Button Text', PayPalDonations::TEXT_DOMAIN),
            ...
            array(
                ...
                'description' => __(
                    'Enter a text for a text-based custom donation button.',
                    PayPalDonations::TEXT_DOMAIN
                ),
            )
        );
    ...
    public function showAmountCallback($args)
    {
        $output = "";
        $optionKey = PayPalDonations::OPTION_DB_KEY;
        $options = get_option($optionKey);
        $output .= "<input class='regular-text' type='checkbox' id='show_amount' ";
        $output .= "name='{$optionKey}[show_amount]'' ";
    ...
    public function buttonTextCallback($args)
    {
        $optionKey = PayPalDonations::OPTION_DB_KEY;
        $options = get_option($optionKey);
